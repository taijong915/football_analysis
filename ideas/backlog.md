...
- **1차 정의 기준 결론 (재실행 대기)**: 침투 준비가 잘 된 팀일수록 대회를 더 오래 남았다(16강 진출 상관 보정 후 +0.27). 단 이 지표는 점유 볼륨과 크게 얽혀 있어(r=+0.54, 점유 통제 시 진출 상관 +0.33 → +0.19) 순수한 "침투 의지"는 아니다. 한국은 관측 조건을 보정하면 침투 선택지가 중위권(3위 → 18위), 오프사이드 위치는 반전(31위 → 8위)한다. **2차 확장 정의로 재실행하면 이 수치들은 모두 바뀐다** - 새 결론이 나오기 전까지 인용하지 말 것.
- **한계**: 관측 완전성이 지표를 직접 밀어 올린다(보이는 상대 1명 시 침투 선택지 0.42명, 9명 시 1.44명 - 보정 필수). 영역 우위와 얽혀 있어 "침투 의지"만 잰 값이 아니다. 3경기 팀 평균 신뢰도가 약 0.59라 특정 팀 등수가 아니라 분포상 위치로만 서술. 라인 앞 가시율도 0.80이지 1.0이 아니라 절대 개수는 과소 집계. 2026 북중미 월드컵은 오픈 데이터에 없어 직접 답할 수 없다. 지표가 낮은 팀에 **두 유형이 섞인다**(밀어붙일 일이 없어서 낮은 모로코형 vs 밀어붙이는데 못 뚫어서 낮은 포르투갈형) - `PLAN.md` 한계 절의 "예외 사례" 참고.
- **남은 작업**: 2차 확장 정의로 재실행(`PLAN.md`의 "2차 확장 재실행" 5단계). 블로그 발행은 재실행 완료까지 보류 - 현재 원고는 1차 정의 서사라 무효가 된다.
