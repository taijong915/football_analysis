...
- **분석 질문**:
  ...
  - (확장 4) 한국은 "라인 앞에서 준비한 선수"가 32팀 중 가장 적으면서 "이미 라인을 넘어가 있던(오프사이드 위치) 선수"는 가장 많았다. 이 대조가 경기별로도 일관되며, 무엇으로 읽을 수 있는가?
...
- **한계**: 영역 우위와 강하게 얽혀 있어 "침투 의지"만 순수하게 잰 값이 아니다(점유 통제 시 진출 상관 +0.33 → +0.19). 상관이지 인과가 아니며, 3경기 팀 평균의 신뢰도가 약 0.59라 분포의 끝(한국 최하위)에 대한 진술로 제한한다. 라인 앞 구역도 가시율이 0.80이지 1.0이 아니므로 절대 개수는 과소 집계다. 2026 북중미 월드컵 데이터가 오픈데이터에 없어, 출발점이었던 "2026 한국은 왜 부진했는가"에는 직접 답할 수 없다.
