...
새 분석 주제는 항상 4단계를 거칩니다: **아이디어 논의(`ideas/backlog.md`, 질문증강 방식) → 조작적 정의 검토 + 데이터 검토(먼저 개념을 숫자로 바꾸는 규칙이 그 개념을 맞게 재는지 경기 규칙 대조·예상 크기·대안 정의·극단 사례로 확인하고, 이어서 방법론이 의존하는 필드의 결측률·분포를 대상 경기/대회로 직접 확인) → 주제별 전용 폴더 생성(`PLAN.md` 포함, 예: `spain_euro2024/`) → 그 폴더 하위 `processed/`에서 산출물 관리**. 조작적 정의는 코드로 옮기기 전에 한 문장으로 적어 사용자에게 확인받고, 결과가 도메인 상식과 어긋나면 데이터 한계로 결론짓기 전에 정의부터 다시 봅니다. 재사용 가능한 함수가 필요해지면 주제 폴더 안에서 바로 만들지 말고 `notebooks/`·`scripts/`·`data/`(공용 샌드박스)에서 먼저 프로토타입·테스트한 뒤 검증되면 `src/`로 승격해 커밋합니다.
